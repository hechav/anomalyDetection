...
class AnomalyPruner:
	...
	def __init__(self, url, es_connection, day_to_search):
		...
		self.random_seed = np.random.randint(10000000)
		self.day_of_data = day_to_search
	def get_traffic(self,day_to_search):
		...
		.drop_duplicates()
		self.url_data = df
	def pruner_trn(self, df=None, max_cats=5, n_trees=300, n_depth=10, feats_cat= ['proto','agent','countrycode','content_type','response']):
	...
	def pruner_predict(self, df=None, model_path=None):
		if df is None:
			df = self.trntst_df
		...
		# self.anomalous_ips = tst_temp.loc[tst_temp.anomaly==1]["clientip"].values.flatten().tolist()
